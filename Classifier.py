...
if __name__ == '__main__':
    ...
    rand_normal = generate_similar([0.2, 0.2], [0.1, 0.1], round(extractionResult.shape[0] / 2))
    ...
    # fastForest = RandomForestClassifier(class_weight=None, max_depth=None, n_estimators=2).fit(X_train, y_train)
    # fast_pred = fastForest.predict(X_test)
    # print(f"Classification report for {fastForest.__class__.__name__} : \n{classification_report(y_test, fast_pred, output_dict=True)}")

    plotter = PyplotWrapper(xlabel='mean', ylabel='std', figsize=(1, 2))
    plotter.scatter([living["mean"].values.tolist()], living["std"].values.tolist(), marker="o", label="Living", c='k')
    plotter.scatter(dead["mean"].values.tolist(), dead["std"].values.tolist(), marker="o", label="Perfect Dead", c='w',
                    edgecolors='k')
    plotter.scatter([sublist[0] for sublist in rand_normal], [sublist[1] for sublist in rand_normal], marker="s",
                    label="Imperfect Dead", c='w', edgecolors='k')
    plotter.show()
